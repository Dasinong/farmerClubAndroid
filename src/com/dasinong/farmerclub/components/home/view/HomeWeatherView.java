...
import com.dasinong.farmerclub.utils.TimeUtils;
...
public class HomeWeatherView extends LinearLayout {
	...
	public static final String TAG = "HomeWeatherView";
...
//	private TextView mOpenSevenDays;
	...
	private void init() {
		...
		mRoot = LayoutInflater.from(getContext()).inflate(R.layout.view_weather, null);
	...
	private void initCurrentWeatherView() {
		...
		mCurrentWind = (TextView) findViewById(R.id.tvWind);
...
//		mOpenSevenDays = (TextView) mRoot.findViewById(R.id.tvCloseWeekTemp);
	...
	private void updateCurrentWeatherView(WeatherEntity.CurrentWeather item) {
		...
		String temp = item.l1;
		mCurrentTemp.setText(temp + "°C");

		mCurrentWind.setText(getCurrentWindDirect(item.l4) + "  " + item.l3 + "级");
		mCurrentWeatherStatus.setText(getWeather(item.l5));
		mCurrentWeatherIcon.setImageResource(getCurrentIconRes(item.l5));
		mCurrentRain.setText("降雨量  " + item.l6 + "毫米");
		mMaxTem.setText("最高  " + item.daymax + "°c");
		mMinTem.setText("最低  " + item.daymin + "°c");
	}
	...
	private int getCurrentIconRes(String level5) {
		...
		Logger.e(TAG, "getCurrentIconRes" + resId + "name" + iconName);
...
//		mOpenSevenDays.setText(getContext().getString(R.string.weather_close_one_week, weatherCount));
	...
	private String getSevenWindLevel(String ddLevel) {
		...
		try {
			...
			Logger.e(TAG, "ddLevle parse error" + ddLevel);
		}
		return windDirect[index];
	}
	...
	private int getSevenWeatherIcon(String weather) {
		...
		Logger.e(TAG, "getSevenWeatherIcon" + resId + "name" + iconName);
