...
import com.dasinong.farmerclub.ui.adapter.VarietiesFirstListAdapter;
import com.dasinong.farmerclub.ui.adapter.VarietiesSecondListAdapter;
...
import com.dasinong.farmerclub.utils.HanziToPingyin;

public class AllVarietyActivity extends BaseActivity {
	...
	private void initData(final String type) {
		...
		new Thread() {
			public void run() {
				EncyclopediasDao dao = new EncyclopediasDao(AllVarietyActivity.this);
	...
	private void setUpView() {
		...
		mSecondList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				intent.putExtra(AddCropActivity.CROP_NAME, crop.cropName);
				intent.putExtra(AddCropActivity.CROP_ID, crop.cropId+"");
				setResult(0, intent);
